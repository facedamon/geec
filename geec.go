...
// RegisterPeers registera a PeerPicker for choosing remote peer
...
func (g *Group) load(key string) (value ByteView, err error) {
	// each key is only fetch once (either locally or remotely)
	...
	viewi, err := g.loader.Do(key, func() (interface{}, error) {
		if g.peers != nil {
			if peer, ok := g.peers.PickPeer(key); ok {
				...
				log.Println("[Geec] Failed to get from peer", err)
			}
		}
